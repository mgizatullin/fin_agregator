...
<head>
    ...
    <title>{{ $card->seo_title ?? ($card->name . ($card->bank ? ' — ' . $card->bank->name : '')) }}</title>

    <meta name="description" content="{{ $card->seo_description ?? $card->short_description ?? \Illuminate\Support\Str::limit(strip_tags($card->description ?? ''), 160) }}">
...
<body>
                            ...
                            <div class="header-inner-wrap">
                                ...
                                <div class="header-right d-flex align-items-center ">
                                    <div class="popup-show-form">
                                        <a href="javascript:void(0)" class="tf-btn btn-white btn-find btn-show ">
    ...
    <script>
        document.querySelectorAll('.card-tariffs-expand-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                var selector = this.getAttribute('data-expand-target');
                if (!selector) return;
                var rows = document.querySelectorAll(selector);
                var expanded = rows.length && rows[0].classList.contains('is-visible');
                var labelExpand = this.getAttribute('data-expand-label') || 'Развернуть';
                var labelCollapse = this.getAttribute('data-collapse-label') || 'Свернуть';
                rows.forEach(function(row) { row.classList.toggle('is-visible', !expanded); });
                this.querySelector('span:first-child').textContent = expanded ? labelExpand : labelCollapse;
            });
        });
    </script>
    <!-- /Javascript -->
