@php
    ...
    $url = $item->slug ? url_canonical(route('loans.category.show', $item->slug)) : (filled($item->website) ? $item->website : '#');
    $materialUrl = $item->slug ? url_canonical(route('loans.category.show', $item->slug)) : '#';
@endphp
...
<div class="tf-box-icon style-7 v2 effect-icon loan-card">
    ...
    <div class="d-flex align-items-center gap_12 flex-wrap">
        @if($materialUrl !== '#')
            <a href="{{ $materialUrl }}" class="card-action__info-btn" title="Подробнее" aria-label="Подробнее">i</a>
        @endif
