@php
    $reviews = $reviewable->reviews->where('is_published', true)->values();
...
<section id="product-reviews" class="section-reviews tf-spacing-27">
    <div class="section-heading">
        <h3 class="section-reviews__title">{{ $sectionTitle }}</h3>
        @if($avgRating)
    ...
    @if($reviews->isNotEmpty())
        <div class="reviews-list">
            @foreach($reviews as $review)
                @php
                    $svc = ($review->service ?? '') === 'deposit' || ($review->service ?? '') === 'Вклад' || empty($review->service) ? $serviceLabel : $review->service;
                @endphp
                <article class="review-card">
                    <header class="review-card__header">
                        <h4 class="review-card__title">{{ $review->title ?: 'Без заголовка' }}</h4>
                        <span class="review-card__header-meta">
                            <span>{{ $review->bank?->name ?? $bankName }}</span>
                            <span>·</span>
                            <span>{{ $svc }}@if($svc === $serviceLabel): {{ $productName }}@endif</span>
                        </span>
    ...
    @else
        <p class="reviews-empty">Пока нет отзывов. Станьте первым, кто поделится мнением.</p>
    @endif

    <div class="review-form-wrapper">
        <h3 class="review-form__title">Оставить отзыв</h3>
        <form action="{{ $formAction }}" method="post" class="review-form product-review-form">
            @csrf
            <div class="review-form__grid">
                <div class="review-form__field">
                    <label class="review-form__label" for="review-title-{{ $formId }}">Заголовок</label>
                    <input id="review-title-{{ $formId }}" type="text" name="title" class="review-form__input" value="{{ old('title') }}">
                </div>
                <div class="review-form__field">
                    <label class="review-form__label">Банк</label>
                    <input type="hidden" name="bank_id" value="{{ $bankId ?? '' }}">
                    <span class="review-form__input review-form__input--static">{{ $bankName ?? '—' }}</span>
                </div>
                <div class="review-form__field">
                    <label class="review-form__label">Услуга</label>
                    <input type="hidden" name="service" value="{{ $serviceLabel }}">
                    <span class="review-form__input review-form__input--static">{{ $serviceLabel }}</span>
                </div>
                <div class="review-form__field review-form__field--full {{ $reviewFormErrors ? 'review-form__field--error' : '' }}">
                    <label class="review-form__label" for="review-body-{{ $formId }}">Отзыв <span class="review-form__required">*</span></label>
                    <textarea id="review-body-{{ $formId }}" name="body" rows="4" class="review-form__textarea" required>{{ old('body') }}</textarea>
                ...
                <div class="review-form__field review-form__field--full {{ $reviewFormErrors ? 'review-form__field--error' : '' }}">
                    ...
                    <div class="review-form-stars" role="group" aria-label="Оценка от 1 до 5">
                        <input type="hidden" name="rating" class="review-rating-value" value="{{ old('rating', '') }}">
                        @for($i = 1; $i <= 5; $i++)
                ...
                <div class="review-form__field {{ $reviewFormErrors ? 'review-form__field--error' : '' }}">
                    <label class="review-form__label" for="review-name-{{ $formId }}">Имя <span class="review-form__required">*</span></label>
                    <input id="review-name-{{ $formId }}" type="text" name="name" class="review-form__input" value="{{ old('name') }}" required>
                    @error('name')<span class="review-form__error-text">{{ $message }}</span>@enderror
                </div>
                <div class="review-form__field {{ $reviewFormErrors ? 'review-form__field--error' : '' }}">
                    <label class="review-form__label" for="review-email-{{ $formId }}">E-mail <span class="review-form__required">*</span></label>
                    <input id="review-email-{{ $formId }}" type="email" name="email" class="review-form__input" value="{{ old('email') }}" required>
                    @error('email')<span class="review-form__error-text">{{ $message }}</span>@enderror
                </div>
                <div class="review-form__field">
                    <label class="review-form__label" for="review-phone-{{ $formId }}">Телефон</label>
                    <input id="review-phone-{{ $formId }}" type="text" name="phone" class="review-form__input" value="{{ old('phone') }}">
...
<script>
(function() {
    document.querySelectorAll('form.product-review-form').forEach(function(form) {
        ...
        function updateStars(value) {
            ...
            buttons.forEach(function(btn, i) { btn.classList.toggle('is-filled', (i + 1) <= value); });
        }
        if (starsWrap && buttons.length) {
            ...
            starsWrap.addEventListener('mouseleave', function() { updateStars(hiddenInput ? parseInt(hiddenInput.value, 10) || 0 : 0); });
            starsWrap.addEventListener('mouseenter', function(e) {
                var btn = e.target.closest('.review-form-stars__btn');
                if (btn) { var v = parseInt(btn.getAttribute('data-value'), 10); buttons.forEach(function(b, i) { b.classList.toggle('is-filled', (i + 1) <= v); }); }
            });
        }
    });
})();
